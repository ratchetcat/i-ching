class IeChing
  ...
  class Hexagram < Array
    ...
    def change
      temp = IeChing::Hexagram.new
      self.each_with_index do | l, index |
        if l[:moving]
          type = IeChing::YANG if l[:type] == IeChing::YIN
          type = IeChing::YIN if l[:type] == IeChing::YANG
    ...
    def to_s
      temp = []
      self.each_with_index do | line, index |
        temp << "---o---" if line[:moving] && line[:type] == IeChing::YANG    
        temp << "---x---" if line[:moving] && line[:type] == IeChing::YIN

        temp << "-------" if !line[:moving] && line[:type] == IeChing::YANG    
        temp << "--- ---" if !line[:moving] && line[:type] == IeChing::YIN
      end
      temp.join("\n")
    end
  ...
  class Caster
    ...
    def cast
      ...
      6.times do | i |
        ...
        case temp
        when 9
          h << { :moving => true, :type => IeChing::YANG } # OLD YANG ---o---           
        when 7
          h << { :moving => false, :type => IeChing::YANG } # YOUNG YANG ---x---
        when 6
          h << { :moving => true, :type => IeChing::YIN } # OLD YIN -------
        when 8
          h << { :moving => false, :type => IeChing::YIN } # YOUNG YIN --- ---
        end
      end
